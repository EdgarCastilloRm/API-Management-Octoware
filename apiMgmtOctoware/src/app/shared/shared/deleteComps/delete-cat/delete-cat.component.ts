...
export class DeleteCatComponent implements OnInit {
  ...
  constructor(private api: DataService, private dialogRef: MatDialogRef<DeleteCatComponent>, @Inject(MAT_DIALOG_DATA) public data: any) { }

  ngOnInit(): void {
  }

  deleteWholeAPI() {
    this.api.deleteCategory(this.data)
    .subscribe({
      next: (res) => {
        this.Toast.fire({
          icon: 'success',
